...
from motion.dataset.nturgbd import NTURGBDDataset, nturgbd_label_map
...
image_path = None
image_size = (480,480)
output_dir = 'nturgbd_SIM_MIM_MOT_patch2_sub30_chn_512_cls_head1'
threshold = None
project = 'runs/train'
weight = '100'

exp_name='nturgbd_SIM_MIM_MOT_patch2_sub30_chn_512_cls_head1'
data_path=''
window=150
batch_size=128
save_path='runs/test'
...
# Load LAFAN Dataset
test_dataset = NTURGBDDataset(data_path="../dataset/experiment/nturgbd/ntu_vibe/", motion_length=150, dataset="test")    
full_proc_label_list = list(nturgbd_label_map.values())
label_map = nturgbd_label_map
...
mot_dim_head = 64
target_joint = 'joint'
model = MoT_patch2_seg_chn(seq_len = seq_len, num_joints=num_joints, sub_seq=30, num_classes=n_classes,  dim=mot_dim, depth=mot_depth, heads=mot_heads, mlp_dim = mot_mlp_dim)
...
print('sljflnvnlvnlvlnsvnlvs',motions.shape)
...
print('valid_length', valid_length.shape)
print(motions[0,0,:valid_length[0]].shape)
for exam_idx in range(119):
    ...
    attentions_joint = nn.functional.interpolate(attentions_joint.unsqueeze(0), scale_factor=(30,1), mode="nearest")[0].cpu().numpy()
    ...
    save_path = os.path.join('nturgbd_patch_sub30_head1_chn_val')
    # if save_path == prev_file :
    #     continue
    Path(save_path).mkdir(parents=True, exist_ok=True)
    # Path(os.path.join(save_path, 'tmp')).mkdir(parents=True, exist_ok=True)
    img_aggr_list = []
    ...
    for t in range(global_pose.shape[0]):
        ...
        # print(origin_data['global_p'].shape)
    ...
    gif_path = os.path.join('nturgbd_patch_sub30_head1_chn_val','nturgbd_patch_sub30_head1_chn_val'+str(prefixx)+'.gif')
