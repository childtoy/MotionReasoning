...
def train(opt, device):
    ...
    mean_6d = torch.Tensor(np.array(train_dataset.padded_dataset['rotation_6d_pose_list']).mean(axis=(0,1,2))).to(device)
    std_6d = torch.Tensor(np.std(train_dataset.padded_dataset['rotation_6d_pose_list'], axis=(0,1,2))).to(device)    
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in pbar:
            ...
            motions = (motions - mean_6d) / std_6d
...
def parse_opt():
    ...
    parser.add_argument('--weight_path', type=str, default='runs/train/humanact12_SIM_MIM_MOT_patch2_norm_512/weights/train-1000.pt', help='pretrained weight path')    
    parser.add_argument('--wandb_pj_name', type=str, default='MoCAM', help='project name')
    parser.add_argument('--batch_size', type=int, default=32, help='batch size')
    parser.add_argument('--epochs', type=int, default=1500)
    parser.add_argument('--device', default='0', help='cuda device')
    parser.add_argument('--entity', default=None, help='W&B entity')
    parser.add_argument('--exp_name', default='humanact12_SIM_MIM_MOT_patch2_512_cls_norm', help='save to project/name')
