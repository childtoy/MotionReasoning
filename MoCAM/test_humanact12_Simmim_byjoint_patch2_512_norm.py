...
def test(opt, device):
    ...
    mean_6d = torch.Tensor(np.array(test_dataset.padded_dataset['rotation_6d_pose_list']).mean(axis=(0,1,2))).to(device)
    std_6d = torch.Tensor(np.std(test_dataset.padded_dataset['rotation_6d_pose_list'], axis=(0,1,2))).to(device)
...
def parse_opt():
    ...
    parser.add_argument('--weight', default='1500')
    parser.add_argument('--exp_name', default='humanact12_SIM_MIM_MOT_patch2_512_cls_norm', help='experiment name')
