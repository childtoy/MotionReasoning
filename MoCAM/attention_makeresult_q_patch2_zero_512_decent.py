...
with torch.no_grad():
    for batch in pbar:
        ...
        attn_org1, attn_inp1 = make_attention(attentions_joint_lerp)
        grayscale_cam = np.transpose(attn_inp1[0])   
        plot_pre_heatmap(grayscale_cam,selected_joint_names, is_save=True, savepath=os.path.join(save_dir,save_filename+'_'+label_list[pred_origin]+'_5.png')) 
        ...
        attn_org1, attn_inp1 = make_attention(attentions_joint_lerp)
        grayscale_cam = np.transpose(attn_inp1[0])   
        plot_pre_heatmap(grayscale_cam,selected_joint_names, is_save=True, savepath=os.path.join(save_dir,save_filename+'_'+label_list[pred_origin]+'_10.png'))
        ...
        # sjoint_idxs, sseq_idxs, sattn_values = find_idx_topk_attention(attn_org1[0], 80, 40)
        # lerp_data = lerp_input_repr(data[0],sseq_idxs,sjoint_idxs,20)
        # attentions_joint_lerp = model(lerp_data.unsqueeze(0).to(device), return_attention=True)
        # output2 = model(lerp_data.unsqueeze(0).to(device))
        # pred_top5 = output2.data.max(1)[1]

        # sjoint_idxs, sseq_idxs, sattn_values = find_idx_topk_attention(attn_org1[0], 80, 60)
        # lerp_data = lerp_input_repr(data[0],sseq_idxs,sjoint_idxs,20)
        # attentions_joint_lerp = model(lerp_data.unsqueeze(0).to(device), return_attention=True)
        # output2 = model(lerp_data.unsqueeze(0).to(device))
        # pred_top6 = output2.data.max(1)[1]

        # sjoint_idxs, sseq_idxs, sattn_values = find_idx_topk_attention(attn_org1[0], 80, 80)
        # lerp_data = lerp_input_repr(data[0],sseq_idxs,sjoint_idxs,20)
        # attentions_joint_lerp = model(lerp_data.unsqueeze(0).to(device), return_attention=True)
        # output2 = model(lerp_data.unsqueeze(0).to(device))
