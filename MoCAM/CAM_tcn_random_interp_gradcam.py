...
def lerp_input_repr(input, target_seqs, target_joints, seq_len):
    ...
    for joint_idx, start_seq in zip(target_joints, target_seqs):
        ...
        output[start_seq:start_seq+seq_len, joint_idx:joint_idx+1] = interpolated.squeeze(0)
    return output
...
save_dir = './result_attention_TCN_GRADCAM_val_lerp_all'
...
gradcam = GradCAM(model,model.tcn.network,
               use_cuda=False)
...
for batch in pbar:
    ...
    target =  [ClassifierOutputTarget(pred_origin) for _ in range(batch_size)]
    grayscale_cam = gradcam(data[0:], targets=target)

    
#         cam_targets =  [ClassifierOutputTarget(cam_target) for _ in range(batch_size)]
    # grayscale_cam = cam(data[0:], targets=cam_targets)        
    ...
    target =  [ClassifierOutputTarget(pred_top3) for _ in range(batch_size)]

    grayscale_cam = gradcam(lerp_data[0:1], targets=target)
    ...
    target =  [ClassifierOutputTarget(pred_top2) for _ in range(batch_size)]
    ...
    target =  [ClassifierOutputTarget(pred_top1) for _ in range(batch_size)]
