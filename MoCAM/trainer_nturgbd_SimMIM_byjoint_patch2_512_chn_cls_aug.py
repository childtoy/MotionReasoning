...
from motion.dataset.nturgbd import NTURGBDDataset2, nturgbd_label_map
...
from model.SImMIM import SimMIM_patch2_seg_chn
import math
def lerp_input_repr(input, valid_len, seq_len):

    amp_const = np.random.randint(5,12,[1])/10
...
def train(opt, device):
    ...
    train_dataset = NTURGBDDataset2(data_path="../dataset/experiment/nturgbd/ntu_vibe/", motion_length=150, dataset="train", transform=transform)
    full_proc_label_list = list(nturgbd_label_map.values())
    print(len(full_proc_label_list))
    ...
    mot = MoT_patch2_seg_chn(seq_len = seq_len, num_joints=num_joints, sub_seq=30, num_classes=n_classes,  dim=mot_dim, depth=mot_depth, heads=mot_heads, mlp_dim = mot_mlp_dim)
...
def parse_opt():
    ...
    parser.add_argument('--weight_path', type=str, default='runs/train/nturgbd_SIM_MIM_MOT_patch2_512_chn_aug/weights/train-100.pt', help='pretrained weight path')    
    ...
    parser.add_argument('--exp_name', default='nturgbd_SIM_MIM_MOT_patch2_chn_512_cls_aug', help='save to project/name')
