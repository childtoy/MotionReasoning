...
def lerp_input_repr(input, valid_len, seq_len):
    ...
    select_const = np.random.randint(1,10,[1])
    dataset = input.copy()
    data = dataset.copy()
    output = data.copy()
    mask_start_frame = 0
...
def parse_opt():
    ...
    parser.add_argument('--weight_path', type=str, default='runs/train/humanact12_SIM_MIM_MOT_patch2_sj_noval_512_augthree/weights/train-100.pt', help='pretrained weight path')    
    ...
    parser.add_argument('--exp_name', default='humanact12_SIM_MIM_MOT_patch2_sj_noval_512_ssl_augthree_cls_noaug', help='save to project/name')
