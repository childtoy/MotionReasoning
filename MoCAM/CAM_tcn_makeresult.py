...
cam = EigenCAM(model=model, target_layers = model.tcn.network, use_cuda=True)
print(data.shape)
...
for i in selected_joint:
    selected_joint_names.append(joint_names[i])
for i in range(100):
    save_path = os.path.join('result_tcn_localq', origin_data['filename'][i].split('.mat')[0])
    ...
    gif_path = os.path.join('result_tcn_localq', str(origin_data['filename'][i].split('.mat')[0])+'_'+str(origin_data['start_frame'][i].numpy())+'_'+prdstr+'.gif')
