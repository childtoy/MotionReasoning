...
with torch.no_grad():
    for batch in pbar:
        ...
        sjoint_idxs, sseq_idxs, sattn_values = find_idx_topk_attention(attn_org1[0], 80, 45)
        ...
        sjoint_idxs, sseq_idxs, sattn_values = find_idx_topk_attention(attn_org1[0], 80, 50)
        lerp_data = lerp_input_repr(data[0],sseq_idxs,sjoint_idxs,20)
        attentions_joint_lerp = model(lerp_data.unsqueeze(0).to(device), return_attention=True)
        ...
        sjoint_idxs, sseq_idxs, sattn_values = find_idx_topk_attention(attn_org1[0], 80, 55)
        lerp_data = lerp_input_repr(data[0],sseq_idxs,sjoint_idxs,20)
        attentions_joint_lerp = model(lerp_data.unsqueeze(0).to(device), return_attention=True)
