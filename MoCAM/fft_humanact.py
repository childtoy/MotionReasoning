...
for exam_idx in range(119):
    ...
    for i in selected_joint:
        selected_joint_names.append(joint_names[i])
    prefixx = str(exam_idx)+'GT:'+ le.inverse_transform([labels[exam_idx].cpu().numpy()])[0] + ' Seq_Len:' +str(valid_length[exam_idx].cpu().numpy())
    i=0
    save_path = os.path.join('result_fft')
    ...
    data = global_pose.cpu().numpy()

    # def fft_transform_alljoint(data):
    data_high5 = data.copy()
    data_low5 = data.copy()
    dlen = valid_length[exam_idx].cpu().numpy()
    t = np.arange(0,dlen, 1)
    fs = dlen
    dt = 1/fs
    x1 = np.arange(0, 1, dt)
    # nfft = 샘플 개수
    ...
    for jt in range(22):
        for ax in range(3):
            # joint rw1 : 8 , x축
            y1 = data[:,jt,ax] - np.mean(data[:,jt,ax])
            ...
            y_high5 = np.zeros((nfft,)) 
            y_low5 = np.zeros((nfft,)) 
            arfreq = [] 
            arcoec = [] 
            arcoes = [] 
            for i in range(0,nfft_half): 
                freq = f0[idxy[i]] 
                yx = fft_y[idxy[i]] 
                coec = yx.real 
                coes = yx.imag * -1 
                if i < 5:
                    y_high5 += coec * np.cos(2 * np.pi * freq * x1) + coes * np.sin(2 * np.pi * freq * x1) 
                if i < 10 and i >5:
                    y_low5 += coec * np.cos(2 * np.pi * freq * x1) + coes * np.sin(2 * np.pi * freq * x1) 
                arfreq.append(freq) 
                arcoec.append(coec) 
                arcoes.append(coes)
            y_high5 = y_high5 + np.mean(data[:,jt,ax])
            y_low5 = y_low5 + np.mean(data[:,jt,ax])
            data_high5[:,jt,ax] = y_high5
            data_low5[:,jt,ax] = y_low5
    img_aggr_list = []
    save_path = './result_fft'
    for t in range(dlen):
        input_img_path = os.path.join(save_path, 'tmp')
        plot_single_pose(data[t],bm, t, minx,maxx,miny,maxy,minz,maxz,input_img_path, 'input', prefixx)
        input_img = Image.open(os.path.join(input_img_path, 'input'+str(t)+'.png'), 'r')
        img_aggr_list.append(input_img)
    # Save imagesa
    gif_path = os.path.join('result_fft','result_fft'+str(prefixx)+'.gif')
    ...
    img_aggr_list = []
    save_path = './result_fft_low5'
    for t in range(dlen):
        input_img_path = os.path.join(save_path, 'tmp')
        plot_single_pose(data_low5[t],bm, t, minx,maxx,miny,maxy,minz,maxz,input_img_path, 'input', prefixx)
        input_img = Image.open(os.path.join(input_img_path, 'input'+str(t)+'.png'), 'r')
        img_aggr_list.append(input_img)
    # Save imagesa
    gif_path = os.path.join('result_fft_low5','result_fft_low5'+str(prefixx)+'.gif')
    imageio.mimsave(gif_path, img_aggr_list, duration=0.1)
    # print(f"ID {exam_idx}: test completed.")
    img_aggr_list = []
    save_path = './result_fft_high5'
    for t in range(dlen):
        input_img_path = os.path.join(save_path, 'tmp')
        plot_single_pose(data_high5[t],bm, t, minx,maxx,miny,maxy,minz,maxz,input_img_path, 'input', prefixx)
        input_img = Image.open(os.path.join(input_img_path, 'input'+str(t)+'.png'), 'r')
        img_aggr_list.append(input_img)
    # Save imagesa
    gif_path = os.path.join('result_fft_high5','result_fft_high5'+str(prefixx)+'.gif')
    imageio.mimsave(gif_path, img_aggr_list, duration=0.1)
    print(f"ID {exam_idx}: test completed.")
