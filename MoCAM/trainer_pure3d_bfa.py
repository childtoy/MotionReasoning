...
from data_proc.mocap_bfa_dataset import BFADataset
from data_proc.utils import increment_path
from model.model import PURE3D, TCN
...
def train(opt, device):
    ...
    emotion_dataset = BFADataset(data_dir=opt.data_path, processed_data_dir=opt.processed_data_dir, train=True, device=device, window=opt.window)
    ...
    kernel_size = 3
    model = PURE3D(input_channels, n_channels, n_classes, kernel_size=kernel_size, dropout=0.2)
    model.to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in pbar:
            ...
            print(local_q.shape)
            print(local_q[:,:,-1,:].shape)
            # local_q = local_q.reshape(batch_size, opt.window, -1)
            # q_vel = q_vel.reshape(batch_size, opt.window, -1)
            # q_acc = q_acc.reshape(batch_size, opt.window, -1)
...
def parse_opt():
    ...
    parser.add_argument('--data_path', type=str, default='/home/taehyun/workspace/childtoy/MotionReasoning/style_transfer/data/mocap_bfa', help='Mat dataset path')
    parser.add_argument('--processed_data_dir', type=str, default='processed_data_bfa/', help='path to save pickled processed data')
