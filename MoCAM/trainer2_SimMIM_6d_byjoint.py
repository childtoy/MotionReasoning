...
def train(opt, device):
    ...
    mot = MoT_6d(seq_len = seq_len, num_joints=num_joints, num_classes=n_classes,  dim=mot_dim, depth=mot_depth, heads=mot_heads, mlp_dim = mot_mlp_dim)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in pbar:
            ...
            loss.backward()
            # torch.nn.utils.clip_grad_norm_(mim.parameters(), 1.0, error_if_nonfinite=False)
...
def parse_opt():
    ...
    parser.add_argument('--exp_name', default='SIM_MIM_MOT_norm2_6D', help='save to project/name')
    parser.add_argument('--save_interval', type=int, default=50, help='Log model after every "save_period" epoch')
    parser.add_argument('--learning_rate', type=float, default=0.0000001, help='generator_learning_rate')
