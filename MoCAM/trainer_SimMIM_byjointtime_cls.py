...
def train(opt, device):
    ...
    ckpt_joint = torch.load(opt.weight_path_joint, map_location=device)
    ckpt_time = torch.load(opt.weight_path_time, map_location=device)
    ...
    mim_joint = SimMIM( encoder = mot_joint, masking_ratio = 0.5)
    ...
    mim_time = SimMIM( encoder = mot_time, masking_ratio = 0.5)
    mim_time.load_state_dict(ckpt_time['SimMIM'])
    mim_joint.load_state_dict(ckpt_joint['SimMIM'])
    mim_time.to(device)
    mim_joint.to(device)


    optim = AdamW(params=list(mot_joint.parameters())+list(mot_time.parameters()), lr=opt.learning_rate)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in pbar:
            ...
            data_joint = local_q
            optim.zero_grad() 
            logits_joint = mot_joint(data_joint.to(device))
            data_time = local_q.permute(0,2,1)
            logits_time = mot_time(data_time.to(device))
            logits = (logits_joint + logits_time) /2
        ...
        if (epoch % save_interval) == 0:
            ckpt = {'epoch': epoch,
                    'SimMIM_time': mim_time.state_dict(),
                    'MoT_time' : mot_time.state_dict(), 
                    'SimMIM_joint': mim_joint.state_dict(),
                    'MoT_joint' : mot_joint.state_dict(), 
                    'seq_len': seq_len,
                    'n_joints': num_joints,
                    'n_heads' : mot_heads,
                    'mlp_dim' : mot_mlp_dim,
...
def parse_opt():
    ...
    parser.add_argument('--weight_path_joint', type=str, default='runs/train/SIM_MIM_MOT38/weights/train-1000.pt', help='pretrained weight path')
    parser.add_argument('--weight_path_time', type=str, default='runs/train/SIM_MIM_MOT_time_1024/weights/train-700.pt', help='pretrained weight path')
    ...
    parser.add_argument('--exp_name', default='SIM_MIM_MOT_jointtime_cls5', help='save to project/name')
