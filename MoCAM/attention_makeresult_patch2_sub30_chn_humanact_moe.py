...
image_path = None
image_size = (480,480)
output_dir = 'humanact12_SIM_MIM_MOT_patch2_512_sub30_cls_chn_moe5'
threshold = None
project = 'runs/train'
weight = '700'

exp_name='humanact12_SIM_MIM_MOT_patch2_512_sub30_cls_chn_moe5'
data_path=''
window=150
batch_size=128
save_path='runs/test'
...
mot_dim_head = 64
target_joint = 'joint'
model = MoT_patch2_seg_chn_moe(seq_len = seq_len, num_joints=num_joints, sub_seq=30, num_classes=n_classes,  dim=mot_dim, depth=mot_depth, heads=mot_heads, mlp_dim = mot_mlp_dim)
...
attentions_joints, gait_weights = model(motions.to(device), valid_length.to(device), return_attention=True)
print(gait_weights)
# for i in gait_weights:
#     print(i)

print(torch.sum(gait_weights))
sys.exit()
...
for exam_idx in range(119):
    ...
    i=0
    save_path = os.path.join('humanact12_patch_sub15_allhead_chn_val')
    ...
    for head_idx in range(8):
        ...
        grayscale_cam = attentions_joint[head_idx]
        ...
        gif_path = os.path.join('humanact12_patch_sub15_allhead_chn_val','humanact12_patch_sub15_allhead_chn_val'+str(head_idx)+'head_'+str(prefixx)+'.gif')
