...
def lerp_input_repr(input, target_seqs, target_joints, seq_len):
    ...
    for joint_idx, start_seq in zip(target_joints, target_seqs):
        ...
        output[:,start_seq:start_seq+seq_len, joint_idx:joint_idx+1] = interpolated
    return output
...
image_path = None
image_size = (480,480)
output_dir = 'SIM_MIM_MOT_patch20_2_cls'
threshold = None
project = 'runs/train'
weight = '500'

exp_name='SIM_MIM_MOT_jointglobalppatch20_2_cls2'
data_path=''
window=80
batch_size=1
...
# emotion_dataset2 = EmotionDataset(data_dir=data_path, processed_data_dir=processed_data_dir, train=True, device=device, window=window)
...
# emo_dataset = TensorDataset(torch.Tensor(gp).to(device), torch.Tensor(gp_scaled).to(device), torch.Tensor(lq).to(device), torch.Tensor(labs).to(device), torch.Tensor(fname).to(device))
...
model = MoT_patch2(seq_len = seq_len, num_joints=num_joints+num_pair, sub_seq=20, num_classes=n_classes,  dim=mot_dim, depth=mot_depth, heads=mot_heads, mlp_dim = mot_mlp_dim)
...
save_dir = './result_attention_patch2_val_lerp'
pbar = tqdm(emotion_data_loader, position=1, desc="Batch")
correct = 0
correct_pred_top3 = []
correct_pred_top2 = []
...
with torch.no_grad():
    for batch in pbar:
        ...
        # lerp_data = lerp_input_repr(data[0],sseq_idxs,sjoint_idxs,20)
...
# print(np.sum(correct_pred_top1))
# print(np.sum(correct_pred_top2))
# print(np.sum(correct_pred_top3))
print(correct)