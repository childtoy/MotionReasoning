...
def test(opt, device):
    ...
    model = MoT_pos(seq_len = seq_len, num_joints=num_joints, num_classes=n_classes,  dim=mot_dim, depth=mot_depth, heads=mot_heads, mlp_dim = mot_mlp_dim)
...
def parse_opt():
    ...
    parser.add_argument('--exp_name', default='SIM_MIM_MOT_globalp_cls5', help='experiment name')
