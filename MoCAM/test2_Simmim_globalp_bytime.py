...
def test(opt, device):
    ...
    model = MoT_pos(seq_len = num_joints, num_joints=seq_len, num_classes=n_classes,  dim=mot_dim, depth=mot_depth, heads=mot_heads, mlp_dim = mot_mlp_dim)    model.load_state_dict(ckpt['MoT'])
    model.to(device)
    model.eval()
    correct = 0
...
def parse_opt():
    ...
    parser.add_argument('--exp_name', default='SIM_MIM_MOT_24_joint_cls7', help='experiment name')
