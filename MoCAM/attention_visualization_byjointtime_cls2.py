...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='SIM_MIM_MOT_jointtime_cls5_900', help='Path where to save visualizations.')
    ...
    attentions_joint = nn.functional.interpolate(attentions_joint.unsqueeze(0), scale_factor=args.patch_size, mode="nearest")[0].cpu().numpy()
    attentions_time = attentions_time.reshape(nh, w_featmap_time, h_featmap_time)
    attentions_time = nn.functional.interpolate(attentions_time.unsqueeze(0), scale_factor=args.patch_size, mode="nearest")[0].cpu().numpy()
    print(attentions_joint.shape)
    ...
    for j in range(nh):
        fname = os.path.join(args.output_dir, "joint-jt-attn-head" + str(j) + ".png")
        plt.imsave(fname=fname, arr=attentions_joint[j], format='png')
        fname = os.path.join(args.output_dir, "time-jt-attn-head" + str(j) + ".png")
