...
def find_idx_topk_attention(data, seq_len=80, select_k=3):
    ...
    for i in top_k_value:
        ...
        joint_list2.append(idxs[1][0])
    #     seq_list.append(int(0))
    joint_list = list(np.random.randint(0,35,3))  
    while(bool(set(joint_list).intersection(joint_list2))):
        joint_list = list(np.random.randint(0,35,3))  
...
def lerp_input_repr(input, target_seqs, target_joints, seq_len):
    ...
    for joint_idx, start_seq in zip(target_joints, target_seqs):
        ...
        for i in range(0,40): 
            ...
            if i < 10 and i >5:
                y_low5 += (coec * np.cos(2 * np.pi * freq * x1) + coes * np.sin(2 * np.pi * freq * x1))[0]
            arfreq.append(freq) 
            arcoec.append(coec) 
            arcoes.append(coes)
            # y_high5 = y_high5 + np.mean(data[:,jt])
            y_low5 = y_low5 + np.mean(data)
            # data_high5 = y_high5
            data_low5 = y_low5
        ...
        output[:,:, joint_idx:joint_idx+1] = torch.tensor(data_low5).unsqueeze(0).unsqueeze(2)
    return output
...
processed_data_dir2 = 'processed_data_mocam_80_All_Class_norm4'
save_path='runs/test'
...
emotion_dataset2 = EmotionDataset(data_dir=data_path, processed_data_dir=processed_data_dir2, train=True, device=device, window=window)
...
mean_q = emotion_dataset2.data['mean_q']
std_q = emotion_dataset2.data['std_q']
zero_q = (torch.zeros([1, 80, 1]) - mean_q)/std_q
print('zero_q', zero_q)
n_classes = 7
seq_len=80
num_joints=35
...
with torch.no_grad():
    for batch in pbar:
        ...
        sjoint_idxs, sseq_idxs, sattn_values = find_idx_topk_attention(attn_org1[0], 80, 3)
        ...
        sjoint_idxs, sseq_idxs, sattn_values = find_idx_topk_attention(attn_org1[0], 80, 4)
        lerp_data = lerp_input_repr(data[0],sseq_idxs,sjoint_idxs,80)
        attentions_joint_lerp = model(lerp_data.unsqueeze(0).to(device), return_attention=True)
