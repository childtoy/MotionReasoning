...
def test(opt, device):
    ...
    model_time = MoT(seq_len = num_joints, num_joints=seq_len, num_classes=n_classes,  dim=mot_dim, depth=mot_depth, heads=mot_heads, mlp_dim = mot_mlp_dim)
    model_time.load_state_dict(ckpt['MoT_time'])
    model_time.to(device)
    model_time.eval()
    model_joint = MoT(seq_len = seq_len, num_joints=num_joints, num_classes=n_classes,  dim=mot_dim, depth=mot_depth, heads=mot_heads, mlp_dim = mot_mlp_dim)
    model_joint.load_state_dict(ckpt['MoT_joint'])
    ...
    with torch.no_grad():
        for batch in pbar:
            ...
            data_joint = local_q
            logits_joint = model_joint(data_joint.to(device))
...
def parse_opt():
    ...
    parser.add_argument('--weight', default='200')
    parser.add_argument('--exp_name', default='SIM_MIM_MOT_jointtime_cls56', help='experiment name')
