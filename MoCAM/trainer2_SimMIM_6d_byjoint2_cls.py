...
from model.MoT import MoT_6d2
from model.SImMIM import SimMIM_6d2
def train(opt, device):
    ...
    mot = MoT_6d2(seq_len = seq_len, num_joints=num_joints*6, num_classes=n_classes,  dim=mot_dim, depth=mot_depth, heads=mot_heads, mlp_dim = mot_mlp_dim)
    mim = SimMIM_6d2( encoder = mot, masking_ratio = 0.5)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in pbar:
            ...
            batch_size, seq_len, num_j, _  = data_6d.shape
            # R_mat = local_R.reshape(-1,3,3)
            # data = transforms.matrix_to_rotation_6d(R_mat)
            data = data_6d.reshape(batch_size, seq_len, num_j, 6)
...
def parse_opt():
    ...
    parser.add_argument('--weight_path', type=str, default='runs/train/SIM_MIM_MOT_6D_norm2_2_cls8/weights/train-1000.pt', help='pretrained weight path')    
    ...
    parser.add_argument('--exp_name', default='SIM_MIM_MOT_6D_norm2_2_cls', help='save to project/name')
