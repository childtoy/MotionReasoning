...
def find_idx_topk_attention(data, seq_len=80, select_k=3):
    ...
    seq_list = []
    joint_list = []
    joint_list = list(np.random.randint(0,140,3))
    seq_list = list(np.random.randint(0,4,3))
...
def lerp_input_repr(input, target_seqs, target_joints, seq_len):
    ...
    for joint_idx, start_seq in zip(target_joints, target_seqs):
        ...
        output[:,start_seq:start_seq+seq_len, joint_idx:joint_idx+1] = torch.zeros([1,20,1])
    return output
...
save_dir = './result_attention_patch2_val_random_interp'
pbar = tqdm(emotion_data_loader, position=1, desc="Batch")
correct = 0
correct1 = 0
correct2 = 0
