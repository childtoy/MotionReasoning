...
from model.MoT import MoT_pos3
from model.SImMIM import SimMIM_pos3
def train(opt, device):
    ...
    mot = MoT_pos3(seq_len = num_joints, num_joints=seq_len*6, num_classes=n_classes,  dim=mot_dim, depth=mot_depth, heads=mot_heads, mlp_dim = mot_mlp_dim)
    mim = SimMIM_pos3( encoder = mot, masking_ratio = 0.5)
    mot.to(device)
    mim.to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in pbar:
            ...
            # data = data.reshape(batch_size, seq_len, num_j, 6p            
            data = global_p
            data = data.permute(0,2,1,3)
            data = data.reshape(batch_size, num_j, -1)                        
            optim.zero_grad()      
...
def parse_opt():
    ...
    parser.add_argument('--exp_name', default='SIM_MIM_MOT_globalp_3', help='save to project/name')
