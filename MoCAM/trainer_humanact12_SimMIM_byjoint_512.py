...
import torch
from model.MoT import MoT_seg
from model.SImMIM import SimMIM_seg
def train(opt, device):
    ...
    mot = MoT_seg(seq_len = seq_len, num_joints=num_joints, num_classes=n_classes,  dim=mot_dim, depth=mot_depth, heads=mot_heads, mlp_dim = mot_mlp_dim)
    mim = SimMIM_seg( encoder = mot, masking_ratio = 0.5)
    mim.to(device)
    mim.train()
...
def parse_opt():
    ...
    parser.add_argument('--exp_name', default='humanact12_SIM_MIM_MOT_joint_512', help='save to project/name')
