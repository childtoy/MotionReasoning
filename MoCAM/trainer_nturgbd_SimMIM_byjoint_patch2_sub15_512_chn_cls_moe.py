...
import torch
from model.MoT import MoT_patch2_seg_chn_moe
from model.SImMIM import SimMIM_patch2_seg_chn_moe
def train(opt, device):
    ...
    train_dataset = NTURGBDDataset(data_path="../dataset/experiment/nturgbd/ntu_vibe/", motion_length=150, dataset="train")
    ...
    mot = MoT_patch2_seg_chn_moe(seq_len = seq_len, num_joints=num_joints, sub_seq=30, num_classes=n_classes,  dim=mot_dim, depth=mot_depth, heads=mot_heads, mlp_dim = mot_mlp_dim)
...
def parse_opt():
    ...
    parser.add_argument('--weight_path', type=str, default='runs/train/nturgbd_SIM_MIM_MOT_patch2_sub30_512_chn_moe3/weights/train-100.pt', help='pretrained weight path')    
    ...
    parser.add_argument('--exp_name', default='nturgbd_SIM_MIM_MOT_patch2_chn_512_cls_moe', help='save to project/name')
